...
from neuronet.resnet1d import FrameBackBone
from timm.models.vision_transformer import Block
from neuronet.utils import get_2d_sincos_pos_embed_flexible
from neuronet.loss import NTXentLoss
from functools import partial


class NeuroNet(nn.Module):
    def __init__(self, fs: int, second: int, time_window: int, time_step: float,
                 encoder_dim, encoder_heads: int, encoder_depths: int,
        ...
        self.autoencoder = MaskedAutoEncoderViT(input_size=self.frame_backbone.feature_num,
                                                embed_dim=encoder_dim, num_patches=self.num_patches,
        ...
        projection_hidden = [encoder_dim] + projection_hidden
        projectors = []
...
class MaskedAutoEncoderViT(nn.Module):
    def __init__(self, input_size: int, num_patches: int,
                 ...
                 decoder_embed_dim: int, decoder_heads: int, decoder_depths: int):
        super().__init__()
        self.patch_embed = nn.Linear(input_size, embed_dim)
...
class NeuroNetEncoderWrapper(nn.Module):
    ...
    def forward(self, x, mode: str = 'cls_token'):
        # frame backbone
        x = self.make_frame(x)
        x = self.frame_backbone(x)
...
if __name__ == '__main__':
    ...
    m0 = NeuroNet(fs=100, second=30, time_window=5, time_step=0.5,
                                 encoder_dim=256, encoder_depths=6, encoder_heads=8,
